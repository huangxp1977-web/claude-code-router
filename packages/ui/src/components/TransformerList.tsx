  ...
  return (
    <div className="space-y-3">
      {transformers.map((transformer, index) => {
        ...
        if (!transformer) {
          return (
            <div key={index} className="flex items-start justify-between rounded-md border bg-card p-4 transition-all hover:shadow-md animate-slide-in hover:scale-[1.01]">
          ...
          return (
            <div className="flex flex-wrap gap-2 max-h-8 overflow-hidden">
              {Object.entries(options).map(([key, value]) => (
                <span 
                  key={key} 
                  className="inline-flex items-center px-2 py-1 rounded-md bg-accent text-xs font-medium text-muted-foreground border"
                >
