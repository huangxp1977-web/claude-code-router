...
服务器构建于 **@thxp/llms** 之上，这是一个通用的 LLM API 转换库，提供了核心的请求/响应转换能力。

### 什么是 @thxp/llms？

`@thxp/llms` 是一个独立的 npm 包（`@thxp/llms`），负责处理：
...
`@thxp/llms` 的当前版本是 `1.0.51`。它作为独立的 npm 包发布，可以独立使用或作为 CCR Server 的一部分使用。
